...
function getUniqueId(value: unknown, name: string, index: number, usedIds: Set<string>): string {
  const requestedId = typeof value === "string" || typeof value === "number" ? String(value).trim() : "";
...
export function getStoredTeamMembers(options: { persist?: boolean; includeDefaults?: boolean } = {}): TeamRecord[] {
