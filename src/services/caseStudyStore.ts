...
function getUniqueValue(value: unknown, fallback: string, used: Set<string>): string {
  const requested = typeof value === "string" || typeof value === "number" ? String(value).trim() : "";
  const seed = requested || fallback;
  let result = seed;
  let suffix = 2;
...
export function getStoredCaseStudies(options: { persist?: boolean; includeDefaults?: boolean } = {}): CaseStudyRecord[] {
