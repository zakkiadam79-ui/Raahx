...
export default function CaseStudiesAdmin() {
  ...
  useEffect(() => {
    ...
    fetchCaseStudiesFromApi()
      .then((remoteStudies) => {
        if (!isMounted) return;
        if (remoteStudies.length > 0 || fallbackStudies.length === 0) {
          setStudies(remoteStudies);
        } else {
          setApiError("The Case Study API returned no records. Run the additive CMS migration before editing production data.");
        }
      })
      .catch((error) => {
